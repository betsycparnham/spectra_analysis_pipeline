...
class fit_analysis_plots():
    def __init__(self,results_dir, title, n_draws,n_tune,n_peaks,plot=True):
        ...
        self.n_tune = n_tune
        self.load_directories(n_draws,n_tune,n_peaks)
        if plot==True:
            self.plot_fits(self.full_freq,self.spectra,self.x_fit,self.f_fit,zoom = True,log = True)
            self.plot_fits(self.full_freq,self.spectra,self.x_fit,self.f_fit,zoom = True,log = False)
            self.plot_fits(self.full_freq,self.spectra,self.x_fit,self.f_fit,zoom = False,log = False)
            self.plot_fits(self.full_freq,self.spectra,self.x_fit,self.f_fit,zoom = False,log = True)
    ...
    def plot_fits(self,fullfreq,spectra,x_fit,f_fit,zoom = True,log = True):
        if zoom:
            fig, axs = plt.subplots(2,1,figsize = (4.5,6),constrained_layout = True)
        ...
        if log:
            if zoom:
                fig.savefig(f"stitched_result_load_d{self.n_draws}_t{self.n_tune}_peaks{self.n_peaks}_zoom.png")
            else:
                fig.savefig(f"stitched_result_load_d{self.n_draws}_t{self.n_tune}_peaks{self.n_peaks}.png")

        else:
            if zoom:
                fig.savefig(f"stitched_result_load_d{self.n_draws}_t{self.n_tune}_peaks{self.n_peaks}_linear_zoom.png")
            else:
                fig.savefig(f"stitched_result_load_d{self.n_draws}_t{self.n_tune}_peaks{self.n_peaks}_linear.png")
